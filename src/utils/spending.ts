...
export function mondayStart(d = new Date()): Date {
  const x = new Date(d);
  x.setHours(0, 0, 0, 0);
  const dow = x.getDay();
...
export function weekRangeMonday(weekOffset = 0, anchor = new Date()): [Date, Date] {
...
function txnTime(txn: TxnLike): number {
...
export function expenseBucketsForWeek(
  txns: TxnLike[],
  weekOffset = 0,
  anchor = new Date(),
): number[] {
  ...
  for (const t of txns) {
    ...
    const od = t.occurredDate instanceof Date ? t.occurredDate : new Date(t.occurred_at ?? 0);
...
export function sumExpensesInWeek(txns: TxnLike[], weekOffset = 0, anchor = new Date()): number {
...
export function todayIndexInWeek(anchor = new Date()): number {
...
export function todayIndexInCurrentWeek(): number {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
export type WeekCompareTone = 'good' | 'warn' | 'muted';

export function weekOverWeekLabel(
  thisTotal: number,
  prevTotal: number,
): { text: string; tone: WeekCompareTone } {
