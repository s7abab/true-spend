...
export function StatsScreen({ categoriesExpense, categoriesIncome, resolveCat, currency = 'INR', onTxnPress }: StatsScreenProps) {
  ...
  return <div style={{ paddingBottom: 8, width: '100%', minWidth: 0, flex: '1 1 auto' }}><div style={{ margin: '12px 16px 0', background: '#EBEBF0', borderRadius: 12, padding: 3, display: 'flex', position: 'relative' }}><div style={{ position: 'absolute', top: 3, bottom: 3, left: `calc(${PERIODS.findIndex((p) => p.id === period) * 25}% + 3px)`, width: 'calc(25% - 3px)', background: '#fff', borderRadius: 9, boxShadow: '0 1px 4px rgba(0,0,0,0.09)' }} />{PERIODS.map((p) => <button key={p.id} type="button" onClick={() => { setPeriod(p.id); setOffset(0); }} style={{ flex: 1, zIndex: 1, padding: '8px 0', border: 'none', background: 'transparent', fontSize: 13, fontWeight: 600, color: period === p.id ? '#0F0F12' : '#ACACB8' }}>{p.label}</button>)}</div><div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px 0' }}><button type="button" onClick={() => setOffset((o) => o - 1)} style={{ width: 36, height: 36, borderRadius: 10, background: '#fff', border: 'none', display: 'grid', placeItems: 'center' }}><IChevLeft size={16} /></button><div style={{ flex: 1, textAlign: 'center' }}><div style={{ fontSize: 15, fontWeight: 700 }}>{info.label}</div><div style={{ fontSize: 11, color: '#ACACB8' }}>{info.sub}</div></div><button type="button" disabled={offset >= 0} onClick={() => setOffset((o) => Math.min(0, o + 1))} style={{ width: 36, height: 36, borderRadius: 10, background: '#fff', border: 'none', display: 'grid', placeItems: 'center', opacity: offset >= 0 ? 0.35 : 1 }}><IChevRight size={16} /></button></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 10, padding: '14px 16px 0' }}><StatTile label="Income" value={formatMoney(summary?.total_income ?? 0, currency)} hint={formatDelta(summary?.total_income ?? 0, prevSummaryQ.summary?.total_income ?? 0)} tone="positive" onClick={() => openTotalDetail('income')} /><StatTile label="Expenses" value={formatMoney(summary?.total_expense ?? 0, currency)} hint={formatDelta(summary?.total_expense ?? 0, prevSummaryQ.summary?.total_expense ?? 0)} tone="neutral" onClick={() => openTotalDetail('expense')} /><StatTile label="Net cashflow" value={`${net >= 0 ? '+' : '−'}${formatMoney(Math.abs(net), currency)}`} hint={formatDelta(net, prevNet)} tone={net >= 0 ? 'positive' : 'negative'} onClick={() => openTotalDetail('all')} /><StatTile label="Savings rate" value={`${savingsRate.toFixed(0)}%`} hint={`${summary?.income_txn_count ?? 0} income txns`} tone={savingsRate >= 20 ? 'positive' : savingsRate < 0 ? 'negative' : 'neutral'} onClick={() => openTotalDetail('all')} /></div><div style={{ margin: '12px 16px 0', background: '#fff', borderRadius: 20, padding: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><div style={{ fontSize: 15, fontWeight: 700 }}>Cashflow trend</div><div style={{ fontSize: 11, color: '#ACACB8' }}>{paceUnit === 'hour' ? 'Hourly' : paceUnit === 'day' ? 'Daily' : 'Monthly'}</div></div><div style={{ height: 210 }}>{trendQ.loading || summaryQ.loading ? <div className="loading-panel-placeholder" style={{ height: '100%', borderRadius: 12 }} /> : <ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#EDEDF2" /><XAxis dataKey="label" tick={{ fontSize: 10, fill: '#8C8C99' }} interval="preserveStartEnd" /><YAxis tick={{ fontSize: 10, fill: '#8C8C99' }} width={42} tickFormatter={(v) => `${Math.round(Number(v) / 1000)}k`} /><Tooltip formatter={tooltipFormatter} /><Area type="monotone" dataKey="income" stroke="#22A06B" fill="#DCFCE7" strokeWidth={2} /><Area type="monotone" dataKey="expense" stroke="#F43F5E" fill="#FFE4E6" strokeWidth={2} /></AreaChart></ResponsiveContainer>}</div></div><div style={{ margin: '12px 16px 0', background: '#fff', borderRadius: 20, padding: 14 }}><div style={{ fontSize: 15, fontWeight: 700 }}>Key insights</div><InsightRow title="Top expense category" value={topExpense ? `${topExpense.cat.label} · ${formatMoney(topExpense.amt, currency)}` : 'No expense yet'} /><InsightRow title="Top income category" value={topIncome ? `${topIncome.cat.label} · ${formatMoney(topIncome.amt, currency)}` : 'No income yet'} /><InsightRow title="Average spending pace" value={`${formatMoney(avgSpend, currency)} / ${paceUnit}`} /><InsightRow title="Average earning pace" value={`${formatMoney(avgIncome, currency)} / ${paceUnit}`} /></div><div className="seg" style={{ margin: '14px 16px 0' }}><div className="seg-thumb" style={{ left: KIND_OPTIONS.findIndex((k) => k.id === statsKind) === 0 ? 3 : `calc(${KIND_OPTIONS.findIndex((k) => k.id === statsKind) * (100 / 3)}%)`, width: 'calc(33.33% - 2px)' }} />{KIND_OPTIONS.map((k) => <button key={k.id} type="button" className={`seg-btn${statsKind === k.id ? ' active' : ''}`} onClick={() => { setStatsKind(k.id); setStatsFilter((prev) => ({ ...prev, kind: k.id, categoryId: null, uncategorizedOnly: false })); }} style={{ color: statsKind === k.id ? '#0F0F12' : '#ACACB8' }}>{k.label}</button>)}</div><div style={{ display: 'flex', gap: 8, margin: '12px 16px 0' }}><div className="search-wrap search-wrap--with-side" style={{ margin: 0, flex: 1, minWidth: 0 }}><ISearch size={15} style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', color: '#ACACB8' }} /><input className="search-input" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Filter categories..." /></div><button type="button" className={`history-filter-btn${hasAdvancedHistoryFilters(statsFilter) ? ' history-filter-btn--active' : ''}`} aria-label="Filters" onClick={() => setFilterSheetOpen(true)}><IFilter size={20} stroke={2} /></button></div>{statsError && <div style={{ textAlign: 'center', padding: '8px 16px', color: '#FF4D6D', fontSize: 13 }}>{statsError}</div>}{(expQ.loading || incQ.loading) ? <div className="loading-panel-placeholder" style={{ margin: '12px 16px 0', borderRadius: 20, height: 190 }} /> : statsKind === 'all' ? <><BreakdownSection title="Spending by category" rows={filteredExpense} total={filteredExpense.reduce((a, s) => a + s.amt, 0)} currency={currency} onRow={openCategoryDetail} /><BreakdownSection title="Income by category" rows={filteredIncome} total={filteredIncome.reduce((a, s) => a + s.amt, 0)} currency={currency} onRow={openCategoryDetail} /></> : <BreakdownSection title={statsKind === 'income' ? 'Income by category' : 'Spending by category'} rows={sortedFiltered} total={breakdownTotal || 1} currency={currency} onRow={openCategoryDetail} />}<HistoryFilterSheet open={filterSheetOpen} applied={{ ...statsFilter, search: debouncedSearch }} categoriesExpense={categoriesExpense} categoriesIncome={categoriesIncome} dateFallback={{ fromIso: info.start.toISOString(), toIsoExclusive: info.end.toISOString() }} onClose={() => setFilterSheetOpen(false)} onApply={(patch) => { setStatsFilter((prev) => ({ ...prev, ...patch })); setStatsKind(patch.kind); }} onClear={() => { setStatsFilter({ ...defaultHistoryListFilter(), kind: 'all' }); setStatsKind('all'); setSearch(''); }} /><AnimatePresence>{detailUi ? <StatsDetailSheet key={detailFilter?.rangeKey ?? detailUi.base.categoryScopeKey} open title={detailUi.title} subtitle={detailUi.subtitle} rows={detailQ.rows} loading={detailQ.loading} loadingMore={detailQ.loadingMore} hasMore={detailQ.hasMore} error={detailQ.error} onLoadMore={detailQ.loadMore} onClose={() => setDetailUi(null)} resolveCat={resolveCat} currency={currency} onTxnPress={onTxnPress ? (txn) => { setDetailUi(null); onTxnPress(txn); } : undefined} /> : null}</AnimatePresence></div>;
}
...
function InsightRow({ title, value }: { title: string; value: string }) { return <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: '1px solid #F3F3F7' }}><div style={{ fontSize: 13, color: '#70707E' }}>{title}</div><div style={{ fontSize: 13, fontWeight: 600, color: '#0F0F12', textAlign: 'right' }}>{value}</div></div>; }
